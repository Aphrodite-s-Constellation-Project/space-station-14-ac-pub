...
namespace Content.Shared.Aphrodite.Avali.Abilities;
...
public abstract class SharedAgillitySkillSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var avaliComp))
        {
            if (!TryComp<StaminaComponent>(uid, out var stamina)
                || !avaliComp.Active
                || Timing.CurTime < avaliComp.NextUpdate
                || !Timing.IsFirstTimePredicted)
                continue;

            avaliComp.NextUpdate = Timing.CurTime + TimeSpan.FromSeconds(avaliComp.Delay);

            _stamina.TryTakeStamina(uid, avaliComp.StaminaDamagePassive);
            if (stamina.StaminaDamage > stamina.CritThreshold * 0.50f)
                ToggleAgility((uid, avaliComp));
        }
    }
}
